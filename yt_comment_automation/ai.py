...
1. 【剔除】非歌曲：talk/雑談/MC/感想/开场/结束/告知/宣伝/スクショタイム/挨拶/自己紹介/あくび/章节标记/只有歌手没歌名/只有时间戳没歌名/口琴间奏标记(ハーモニカ、ハーモニカ2、謎の困惑からのハーモニカ2 这类无歌手的口琴标记)/结束语(おつ/ありがとう等)。**活动接力时段表（「18:00~18:30　出场者」「開会式/閉会式」等钟点范围+人名/环节名）是节目单不是歌，也一律剔除。**
2. 【剔除拟声碎片】纯半角片假名且很短（1-2 字符）或单字符平片假名（ｺｯ/ﾋﾟﾖ/ｳﾝ/コ/ッ 这类直播怪声/打断标记）即使原文有时间戳也不是歌名。
3. 【补充】原始时间轴里明确是"时间戳+歌名（可带歌手）"但候选漏掉的行，按原时间戳补上。
4. 【校正】歌名/歌手错字、双重编号残留（「14.14.歌名」→ 剥成纯歌名，编号由格式统一生成）、括号误留（罗马字对照括号删，正式名称括号 ryo(supercell)/久住小春（モーニング娘。）保留）、分隔符统一为半角 " - "。
5. 【格式】每行：时间戳 NN. 歌名 - 歌手 或 时间戳 NN. 歌名。编号从 01 连续。按时间戳升序。
6. 只输出最终列表，禁止任何说明。若确认无任何歌名，只输出：请提供歌名信息后再处理。
...
6. 带时间戳但内容不是歌的**章节/活动标记**，即使格式像歌曲行也要跳过：
   ...
   ハーモニカ（口琴间奏，无歌手时）、あくび、口琴。
7. **活动接力/联合歌枠的时段表不是歌曲**：形如「18:00~18:30　出场者名」「19:30〜20:00　XXX」「開会式/閉会式」
...
3. 原文条目自带编号（如「14.あなたの夜が明けるまで」）时，剥离该编号只保留歌名——严禁输出双重编号（如「14.14.あなたの夜が明けるまで」）。
4. 禁止输出空行、标题、前言、后记、代码块标记、括号补充、备注。
4. 除结果列表外不能输出任何其他内容。
